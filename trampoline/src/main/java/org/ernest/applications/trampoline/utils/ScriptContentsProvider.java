...
public class ScriptContentsProvider {
    ...
    public static String getJar(String jarLocation){
        return "java -Dserver.port=#port "
                + "-Dendpoints.shutdown.enabled=true -Dmanagement.security.enabled=false -Dmanagement.info.git.mode=full -Dmanagement.endpoints.web.exposure.include=* -Dmanagement.endpoint.shutdown.enabled=true #vmArguments -jar \""+jarLocation+"\"";
    }
}
